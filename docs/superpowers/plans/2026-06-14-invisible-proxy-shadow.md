...
4. **Кастер: B1 — простой `staticProxy`; полноценный `ProxyRig` — в Phase C (пересмотр B2/B13/M1).** B1 для alignment-цели нужен лишь ОДИН невидимый статичный кастер (`staticProxy(F,H)` → `THREE.Group` с капсулой-столбиком, `castShadow=true, colorWrite=false, depthWrite=false, visible=true`). Класс `ProxyRig` (пул капсул с per-segment радиусами — рука 0.05, торс 0.12, голова-сфера; `get object(): THREE.Group`; `update(poseWorld,F,H)`) строится **в Phase C** (C.4/C.5), где он реально драйвится позой. `ShadowScene3D` (B1.6) держит кастер свопабельно — поле `caster: THREE.Object3D` + метод `setCaster(obj)` (в B1 = `staticProxy`); **в Phase C** добавляется публичное поле `proxyRig: ProxyRig`, и C.6 делает `setCaster(proxyRig.object)` + драйв `proxyRig.update(...)`. Так B1 остаётся минимальным (YAGNI), а B13 снят: ProxyRig и ссылки на него (`shadowScene3D.proxyRig`) появляются вместе в C, без висячей ссылки. Базис: капсула стоит вдоль **+Z** (Z-up): локальная ось капсулы Y → `rotation.x=+π/2`; центр на `z = F.z + H/2`; корень группы в RAW F (без свопа).
