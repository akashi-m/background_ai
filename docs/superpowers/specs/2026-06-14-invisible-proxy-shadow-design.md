...
**Решение по сложности (YAGNI, разнесено по фазам):** receiver делается в два захода.

- **B1 (стартовый, простой): box-receiver.** Плоскость пола на `floorZ` + 2–3 axis-aligned box-прокси под мебель/стену (координаты из `lights.json` или выставлены руками по планировке B2-10). Материал — **`THREE.ShadowMaterial`**, `receiveShadow=true`. Это даёт ~80% эффекта «тень взбирается на мебель» без tear-culling / нормалей / Uint32. На этом receiver’е делаются alignment (B1) и drive-proxy (C).
...
13. **Receiver: B1 box-прокси сначала, B2 EXR-mesh — отдельной фазой** (YAGNI, §4.1).
